...
# -----
# Insert read books into library.list_books
# -----
...
insert into library.list_books (title, author_id, grade_id, genre_id, comment)
    ...
    where a.last_name='шойнеманн' and a.first_name='фрауке'
        and ge.title='детектив'
        and gr.grade=5;
insert into library.list_books (title, author_id, grade_id, genre_id, comment)
    select 'дом из зелёного стекла', a.id, gr.id, ge.id, 'невероятно захватывающая концовка'
    from list_authors a, list_genres ge, list_grades gr
    where a.last_name='милфорд' and a.first_name = 'кейт'
...
insert into library.list_books (title, author_id, grade_id, genre_id, comment)
    ...
    from list_authors a, list_genres ge, list_grades gr
    where a.last_name='вебб' and a.first_name = 'холли'
        and ge.title='приключения'
        and gr.grade=4;
insert into library.list_books (title, author_id, grade_id, genre_id, comment)
    select 'записки для моих потомков', a.id, gr.id, ge.id, 'очень смешная книга'
    from list_authors a, list_genres ge, list_grades gr
    where a.last_name='борисова' and a.first_name = 'ариадна'
        and ge.title='приключения'
        and gr.grade=5;
insert into library.list_books (title, author_id, grade_id, genre_id, comment)
...
insert into library.list_books (title, author_id, grade_id, genre_id, comment)
    ...
    from list_authors a, list_genres ge, list_grades gr
    where a.last_name='кинг-смит' and a.first_name = 'дик'
        and ge.title='фэнтези'
        and gr.grade=5;
insert into library.list_books (title, author_id, grade_id, genre_id, comment)
